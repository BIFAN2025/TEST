...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Chat
  initChat();
  // Clock: update immediately and every second
  updateClock();
...
// ----- Clock -----
/**
 * Updates the clock display with the current date and time in the
 * Asia/Seoul timezone. The display shows the day of the week, date and
 * time, refreshing every second.
 */
function updateClock() {
  ...
  if (display) {
    display.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;gap:0.3rem;">
      <p>${dateStr}</p>
      <p style="font-size:2rem;font-weight:600;">${timeStr}</p>
    </div>`;
  }
}
...
// ----- Pomodoro Timer -----
/*
  Implements a simple Pomodoro timer. The timer counts down from 25 minutes,
  with buttons to start, pause and reset the countdown. When the time
  reaches zero, an alert is displayed. The display is updated every
  second while the timer is running. This feature helps users focus on
  tasks by breaking work into manageable intervals.
*/
...
/**
 * Updates the Pomodoro display element with the current time left.
 */
function updatePomodoroDisplay() {
  const display = document.getElementById('pomodoro-display');
  if (display) {
    display.textContent = formatTime(pomodoroTimeLeft);
  }
}
...
function startPomodoro() {
  ...
  pomodoroInterval = setInterval(() => {
    if (pomodoroTimeLeft > 0) {
      ...
      alert('포모도로 세션이 끝났습니다! 잠깐 휴식하세요.');
    }
  }, 1000);
}

/**
 * Pauses the Pomodoro timer if it is running.
 */
...
/**
 * Resets the Pomodoro timer back to its initial duration and stops it if running.
 */
...
/**
 * Initialises the Pomodoro timer by assigning event listeners and updating
 * the display. Called on DOMContentLoaded.
 */
